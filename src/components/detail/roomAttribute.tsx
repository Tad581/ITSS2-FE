...
export default function RoomAttribute(props: RoomInfoGridProps) {
  ...
  const items = useMemo(() => {
    ...
    if (haveWifi)
      _items.push(
        <Item key={_items.length} icon={WifiOutlinedIcon}>
          Wifi
        </Item>
      );
    if (haveHeater)
      _items.push(
        <Item key={_items.length} icon={WaterDropOutlinedIcon}>
          Nóng lạnh
        </Item>
      );
    if (haveAirConditioner)
      _items.push(
        <Item key={_items.length} icon={HeatPumpOutlinedIcon}>
          Điều hòa
        </Item>
      );
    if (haveFridge)
      _items.push(
        <Item key={_items.length} icon={KitchenOutlinedIcon}>
          Tủ lạnh
        </Item>
      );
    if (haveWashingMachine)
      _items.push(
        <Item key={_items.length} icon={LocalLaundryServiceOutlinedIcon}>
          Máy giặt
        </Item>
      );
    if (havePCCC)
      _items.push(
        <Item key={_items.length} icon={WhatshotOutlinedIcon}>
          PCCC
        </Item>
      );
    return _items;
