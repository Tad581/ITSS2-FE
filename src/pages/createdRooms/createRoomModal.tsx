...
export default function CreateRoomModal(props: CreateRoomModalProps) {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (formValue) => {
      ...
      formData.append('electronicPrice', formValue.electronicPrice as unknown as string);
      ...
      formData.append('wifiInternet', formValue.wifiInternet as unknown as string);
      formData.append('washingMachine', formValue.washingMachine as unknown as string);
      formData.append('airConditioner', formValue.airConditioner as unknown as string);
      formData.append('waterHeater', formValue.waterHeater as unknown as string);
      formData.append('refrigerator', formValue.refrigerator as unknown as string);
      formData.append('safedDevice', formValue.safedDevice as unknown as string);
      formData.append('enclosedToilet', formValue.enclosedToilet as unknown as string);
  ...
  React.useEffect(() => {
    (async () => {
      if (props.isOpen && props.roomId) {
        ...
        formik.setFieldValue("electronicPrice", room?.data?.roomAttribute?.electronicPrice);
        formik.setFieldValue("waterPrice", room?.data?.roomAttribute?.waterPrice);
        formik.setFieldValue("description", room?.data?.roomAttribute?.description);
        formik.setFieldValue("wifiInternet", room?.data?.wifiInternet);
        formik.setFieldValue("washingMachine", room?.data?.roomAttribute?.washingMachine);
        formik.setFieldValue("airConditioner", room?.data?.roomAttribute?.airConditioner);
        formik.setFieldValue("waterHeater", room?.data?.roomAttribute?.waterHeater);
        formik.setFieldValue("refrigerator", room?.data?.roomAttribute?.refrigerator);
        formik.setFieldValue("safedDevice", room?.data?.roomAttribute?.safedDevice);
        formik.setFieldValue("enclosedToilet", room?.data?.roomAttribute?.enclosedToilet);

        setExistedImages(room?.data?.roomImages?.map((img: any) => img.imageUrl))
      }
    })()
      ...
      >
        <form>
          <Box sx={style}>
            ...
            <ComboBox title='Nhà vệ sinh' label='Chọn loại phòng bạn muốn đăng' options={enclosedToiletOptions} name={'enclosedToilet'} value={formik.values.enclosedToilet} onChange={formik.handleChange} />
            {/*  */}
            ...
            <FormGroup>
              <Grid container columnSpacing={4} width={'100%'}>
                {/* <Box display={'flex'} flexDirection={'row'} justifyContent={'space-between'} alignItems={'center'} mt={2}> */}
                <Grid item xs={3}>
                  <FormControlLabel control={<Checkbox name={'airConditioner'} checked={formik.values.airConditioner} onChange={formik.handleChange} />} label="Điều hòa" />
                </Grid>
                <Grid item xs={3}>

                  <FormControlLabel control={<Checkbox name={'waterHeater'} checked={formik.values.waterHeater} onChange={formik.handleChange} />} label="Nóng lạnh" />
                </Grid>
                <Grid item xs={3}>
                ...
                <Grid item xs={3}>
                  <FormControlLabel control={<Checkbox name={'wifiInternet'} checked={formik.values.wifiInternet} onChange={formik.handleChange} />} label="Wifi" />
                </Grid>
                <Grid item xs={3}>
                  <FormControlLabel control={<Checkbox name={'washingMachine'} checked={formik.values.washingMachine} onChange={formik.handleChange} />} label="Máy giặt" />
                </Grid>
                {/* </Box> */}
              </Grid>
            </FormGroup>
